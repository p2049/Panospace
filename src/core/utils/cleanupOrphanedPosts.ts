import { collection, getDocs, deleteDoc, doc, updateDoc, query, orderBy, limit, startAfter, type QueryDocumentSnapshot } from 'firebase/firestore';
...
export const cleanupOrphanedPosts = async (batchSize = 100) => {
    ...
    while (hasMore) {
        ...
        for (const postDoc of snapshot.docs) {
            checkedCount++;
            const post = postDoc.data() as Partial<Post>;

            // Get the primary image URL
            // Accessing images safely - Post type usually has items[], but legacy might have images[] or imageUrl
...
export const cleanupOrphanedShopItems = async (batchSize = 100) => {
    ...
    while (hasMore) {
        ...
        for (const itemDoc of snapshot.docs) {
            checkedCount++;
            const item = itemDoc.data() as Partial<ShopItem>;
