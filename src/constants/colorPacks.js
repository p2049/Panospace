export const FREE_COLOR_PACK = [
    ...
    // Default Panospace Green (Mint) - Keeping it as a base option usually, but user didn't explicitly list it in "Free Pack" list to ADD, 
    // but implied "Free Colors (Base Pack)". The default #7FFFD4 (Ice Mint) should probably be available.
    // The user listed "Aurora Mint" as Premium.
    // I will add the default Mint as well to be safe, or assume it's the "Ice Mint" name used in existing code?
    // Existing code uses #7FFFD4. I'll add it as "Panospace Mint" or similar if not present. 
    // Wait, the request says "Aurora Mint (Brighter than regular accent)" is premium.
    // I will add the standard #7FFFD4 as "Classic Mint" to Free Pack to ensure continuity.
    { id: 'classic-mint', name: 'Classic Mint', color: '#7FFFD4', isPremium: false }
];

export const PREMIUM_COLOR_PACK = [
...
export const ALL_COLORS = [...FREE_COLOR_PACK, ...PREMIUM_COLOR_PACK];

export const getColorById = (id) => ALL_COLORS.find(c => c.id === id) || FREE_COLOR_PACK.find(c => c.id === 'classic-mint');

// Helper to check if a color string matches a premium color
export const isPremiumColor = (colorValue) => {
